...
// Color tint for each domain_age_label value.
// very_new gets amber — same family as disputed, signals "worth noticing".
// relatively_new and unavailable get neutral gray — understated by design.
// established gets green — a reassuring signal.
const AGE_LABEL_COLOR = {
  very_new:       '#d29922',
...
function ResultCard({ result, onReset }) {
  ...
  return (
    ...
    <div className="result-card">
      ...
      {/* Domain age context — between main explanation and per-source breakdown */}
      <DomainAgeBlock
        domainAgeDays={result.domain_age_days}
        domainAgeLabel={result.domain_age_label}
        domainAgeNote={result.domain_age_note}
      />
...
// ─── DomainAgeBlock ───────────────────────────────────────────────────────────
// Shows domain age as a human-readable string + a short contextual note.
// Color is driven by domain_age_label — not a verdict factor, just context.

// Convert domain_age_days to a friendly string: "14 days", "3 months", "6 years".
function humanAge(days) {
  if (days === null || days === undefined) return 'Unavailable'
...
function DomainAgeBlock({ domainAgeDays, domainAgeLabel, domainAgeNote }) {
  const color = AGE_LABEL_COLOR[domainAgeLabel] ?? '#8b949e'

  return (
    <div className="domain-age">
      <div className="domain-age__header">
        <span className="domain-age__label">Domain Age</span>
        <span className="domain-age__value" style={{ color }}>
          {humanAge(domainAgeDays)}
        </span>
      </div>
      {domainAgeNote && (
        <p className="domain-age__note">{domainAgeNote}</p>
      )}
    </div>
  )
}
